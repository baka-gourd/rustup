...
...
error: could not amend shell profile[..]
...
"#]])
            .is_err();
    }
    ...
    async fn uninstall_keeps_source_in_rcs_when_cargo_bin_is_non_empty() {
        ...
        cx.config.expect(&INIT_NONE).await.is_ok();
        fs::write(cx.config.cargodir.join("bin/custom-tool"), "").unwrap();
        cx.config
            .expect(&["rustup", "self", "uninstall", "-y"])
            .await
            .is_ok();
...
mod windows {
    ...
    async fn uninstall_keeps_path_when_cargo_bin_is_non_empty() {
        ...
        cx.config.expect(&INIT_NONE).await.is_ok();
        std::fs::write(cx.config.cargodir.join("bin/custom-tool"), "").unwrap();
        cx.config
            .expect(&["rustup", "self", "uninstall", "-y"])
            .await
            .is_ok();
    ...
    async fn install_uninstall_affect_path_with_non_unicode() {
        ...
        USER_PATH.set(Some(&reg_value), Some(&uuid)).unwrap();
        ...
        cx.config.expect(&INIT_NONE).await.is_ok();
        assert_eq!(get_path(Some(&uuid)).unwrap().unwrap(), expected);

        cx.config
            .expect(&["rustup", "self", "uninstall", "-y"])
            .await
            .is_ok();
        assert_eq!(get_path(Some(&uuid)).unwrap().unwrap(), reg_value);
    }
}
